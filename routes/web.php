...
Route::get('/', 'App\Http\Controllers\Summary\IndexController')->name('summary.index')->middleware('index'); //MAIN PAGE
...
Route::group(['prefix'=>'resizer','namespace'=>'App\Http\Controllers\Resizer'],function(){
    Route::get('/', 'IndexController')->name('resizer.index');
    Route::get('/{token}', 'ResultController')->name('resizer.result');
    Route::get('/zip/{token}','ZipController')->name('resizer.zip');
});
